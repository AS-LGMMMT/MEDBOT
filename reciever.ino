...
const byte address[6] = "00001"; //creates communication between the two nrf modules we need to create a pipe with an address. 
...
void setup()//setup code which will run once
{
Serial.begin(9600);// commence serial communication baud rate i.e. 9600 
radio.begin();
radio.openReadingPipe(0,address);// opening the pipe. 
radio.setPALevel(RF24_PA_MIN);// It defines minimum amount of transmitting power which helps in draining minimum power consumption we can also set it to maximum as to attain maximum range.
radio.setDataRate(RF24_250KBPS);//  the data rate of transmission 250 KB is defined
radio.startListening();
pinMode(ENA, OUTPUT);//for turning the motors ON, OFF and control its speed
pinMode(ENB, OUTPUT);//for turning the motors ON, OFF and control its speed
pinMode(MotorA1, OUTPUT);//for controlling motor A1
